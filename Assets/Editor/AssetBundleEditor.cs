...
public class AssetBundleEditor : MonoBehaviour
{
    ...
    static void CopyResToRoot(DirectoryInfo rootDir)
    {
        ...
        {
            // 清理旧资源文件
            foreach (var fileInfo in FileUtil.Instance.GetChildFiles(rootDir.FullName))
            {
                fileInfo.Delete();
            }
        }
    ...
    [MenuItem("Tool/AssetBundle/Open Folder/AssetBundles Folder")]
    static void OpenABFolder()
    {
        OpenFolder(GameConst.ASSETBUNDLES_ROOT);
    }
    [MenuItem("Tool/AssetBundle/Open Folder/Build Folder")]
    static void OpenBuildFolder()
    {
        OpenFolder(GameConst.BUILD_ROOT);
    }
    [MenuItem("Tool/AssetBundle/Open Folder/Asset Folder")]
    static void OpenAssetFolder()
    {
        OpenFolder(GameConst.ASSET_ROOT);
    }
    [MenuItem("Tool/AssetBundle/Open Folder/WebDownload Folder")]
    static void OpenWebDownloadFolder()
    {
        OpenFolder(GameConst.WEB_DOWNLOAD_ROOT);
    }
    [MenuItem("Tool/AssetBundle/Build/Build")]
    static void Build()
    {
        // 准备工作 创建需要用到的文件夹
        ...
        List<AssetFileVersion> assetVModelList = new List<AssetFileVersion>();
        ...
        foreach (var file in new DirectoryInfo(GameConst.ASSETBUNDLES_ROOT).GetFiles())
        {
            ...
            bytes = EncryptUtil.Instance.AesEncryptBytes(FileUtil.Instance.ReadBytes(file.FullName));
            ...
            //build version File
            assetVModelList.Add(new AssetFileVersion() { name = name, fileName = fileName, size = size, hash = hash });
        }
        // 导出AB包依赖信息
        ...
        // 导出版本信息
        // 此处后期可做成可视化视图
        AssetVersion vModel = new AssetVersion();
        ...
        string[] defaultFiles = new string[] { "Version", "AssetBundleRely", "lua", "pb", "ui", "p1", "view" };
    ...
    [MenuItem("Tool/AssetBundle/Build/Build ＆ CopyTo AssetRoot")]
    ...
    [MenuItem("Tool/AssetBundle/Build/Build ＆ CopyTo WebDownloadRoot")]
    static void BuildAndCopy2()
    {
        Build();
        CopyResToRoot(new DirectoryInfo(GameConst.WEB_DOWNLOAD_ROOT));
    }
}
